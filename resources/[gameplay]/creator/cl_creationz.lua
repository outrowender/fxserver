    ...
    for i, v in ipairs(character.overlays.order) do
        ...
        if character.overlays[v].colorType ~= 0 then
            ...
            SetPedHeadOverlayColor(
                GetPlayerPed(-1),
                i - 1,
                character.overlays[v].colorType,
                character.overlays[v].color1,
                color
            )
        end
    end
        ...
        elseif WarMenu.IsMenuOpened("_overlaysDesc") then
            ...
            if change then
                SetPedHeadOverlay(
                    PlayerPedId(),
                    activeMenu - 1,
                    _ItemIndex[activeMenu] - 1,
                    _ItemOpacity[activeMenu] / 100
                )
                change = false
